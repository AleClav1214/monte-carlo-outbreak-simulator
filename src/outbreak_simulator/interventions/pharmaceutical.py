...
def sars_cov_2_vaccination(coverage: float = 0.7, variant_era: str = "delta") -> Intervention:
    ...
    if variant_era == "delta":
        ...
        source = (
            "Netherlands household-contact study, Delta era (63% reduction in transmission "
            "to unvaccinated contacts; range reflects broader 20-63% literature)"
        )
    elif variant_era == "omicron":
        mult, low, high = 1 - 0.35, 1 - 0.60, 1 - 0.10
        source = (
            "Multiple Omicron-era studies reporting substantially lower and more "
            "heterogeneous transmission-blocking effectiveness (range ~10-60% "
            "depending on booster timing and age)"
        )
    else:
    ...
    return Intervention(
        ...
        source=source,
        description=(
            "Effectiveness specifically against transmitting to others if infected -- "
            "distinct from (and generally lower than) effectiveness against acquiring infection."
        ),
    )


def generic_vaccination(
    efficacy: float, efficacy_low: float, efficacy_high: float, coverage: float, source: str
) -> Intervention:
